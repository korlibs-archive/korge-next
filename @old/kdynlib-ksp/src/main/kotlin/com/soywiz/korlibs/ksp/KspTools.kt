...
fun KSTypeReference?.toMyType(): MyType {
    ...
    val typeName = StringBuilder(resolvedType.declaration.simpleName.asString())
...
data class MyType(val type: String) {
    val interopType = when (type) {
        "com.soywiz.kdynlib.VoidPtr?" -> "kotlinx.cinterop.COpaquePointer?"
        "com.soywiz.kdynlib.VoidPtr" -> "kotlinx.cinterop.COpaquePointer"
        else -> type
    }
    val isNullable get()= type.endsWith("?")
    val isPointer get() = type.startsWith("com.soywiz.kdynlib.VoidPtr")
