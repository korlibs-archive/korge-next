            ...
            symbol.accept(
                object : KSVisitorVoid() {
                    override fun visitClassDeclaration(classDeclaration: KSClassDeclaration, data: Unit) {
                        ...
                        when {
                            isJvm -> {
                                file.appendln("class $className(val libName: String) : $interfaceName by com.sun.jna.Native.load(libName, $interfaceName::class.java)")
                                file.appendln("operator fun com.soywiz.kmem.lib.LibraryCompanion<$interfaceName>.invoke(name: String): $interfaceName = $className(name)")
                            }
                            isDummy -> {
                                file.appendln("operator fun com.soywiz.kmem.lib.LibraryCompanion<$interfaceName>.invoke(name: String): $interfaceName = TODO(\"Can't instantiate '$interfaceName' in this target\")")
                            }
                            isNative -> {
                                file.appendln("import kotlinx.cinterop.*")
                                file.appendln("class $className(val libName: String) : $interfaceName {")
                                file.appendln("  inline private fun com.soywiz.kmem.lib.VoidPtr.convert() = this.toLong().toCPointer<CPointed>()")
                                file.appendln("  inline private fun COpaquePointer.convert() = this.rawValue")
                                run {
                                    file.appendln("  val __LIB__ = com.soywiz.kmem.DynamicLibrary(libName)\n")
                            ...
                            isMetadata -> {
                                file.appendln("expect operator fun com.soywiz.kmem.lib.LibraryCompanion<$interfaceName>.invoke(name: String): $interfaceName")
                            }
                        }
                    }
                }, Unit
